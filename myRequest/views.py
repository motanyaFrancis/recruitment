...
class UserObjectMixins(object):
    ...
    WEB_PORTAL = 'http://20.231.15.166:7047/BC140/WS/CRONUS%20International%20Ltd./Codeunit/CuRecruitmentWebPortal'

    def verificationToken(self, tokenRange):
        alphabet = string.ascii_letters + string.digits
        SecretCode = ''.join(secrets.choice(alphabet)
                             for i in range(tokenRange))
        return SecretCode
    ...
    def upload_attachment(self, *params):
        ...
        if not session:
            ...
            session.auth = HTTPBasicAuth(
                config.WEB_SERVICE_UID, config.WEB_SERVICE_PWD)
        with ThreadPoolExecutor() as executor:
            client = Client(self.WEB_PORTAL,
                            transport=Transport(session=session))
            response = executor.submit(
                client.service['FnUploadAttachedDocument'], *params).result()
        return response
    ...
    def delete_attachment(self, *params):
        ...
        with ThreadPoolExecutor() as executor:
            client = Client(self.WEB_PORTAL,
                            transport=Transport(session=session))
            response = executor.submit(
                client.service['FnDeleteDocumentAttachment'], *params).result()
        return response
    ...
    def download_attachment(self, *params):
        ...
        with ThreadPoolExecutor() as executor:
            client = Client(self.WEB_PORTAL,
                            transport=Transport(session=session))
            response = executor.submit(
                client.service['FnGetDocumentAttachment'], *params).result()
        return response
